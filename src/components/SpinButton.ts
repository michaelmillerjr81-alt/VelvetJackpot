...
export class SpinButton {
  ...
  private build(): void {
    ...
    const glow = new Graphics().circle(0, 0, R + 14).fill({ color: 0xf5c842, alpha: 0.25 });
    ...
    gsap.to(glow, { alpha: 0.55, duration: 1.1, yoyo: true, repeat: -1, ease: 'sine.inOut' });
  ...
  setSpinning(spinning: boolean): void {
    ...
    this.bg.clear().circle(0, 0, 56).fill({ color: spinning ? 0x1a55a0 : 0xc8172b });
